...
import { WithSize } from '../mixins/size.js';

export const EzAppbarName = 'ez-appbar';

export const EzAppbarEvents: Record<string, string> = {
  Intersected: `${EzAppbarName}-intersected`,
  NotIntersected: `${EzAppbarName}-not-intersected`,
};

const { Intersected, NotIntersected } = EzAppbarEvents,
  ...
  EzAppbarBase = WithSize(EzBaseElement);

export class EzAppbarElement extends EzAppbarBase {
  static localName = EzAppbarName;
  ...
  static properties = {
    ...(EzAppbarBase as unknown as typeof EzBaseElement).properties,
  ...
  set hiddenClassName(str: string) {
  ...
  set visibleClassName(str: string) {
  ...
  set marginTop(n: number) {
    const prevValue = this.marginTop;
  ...
  override willUpdate(_changed: PropertyValues) {
    // Apply variety control classes to host
    if (_changed.has(VARIETY_NAME)) {
      const old = _changed.get(VARIETY_NAME) as string;
      ...
      if (this.variety)
        this.variety
          ...
          .filter(Boolean)
          .forEach(t => {
            this.classList.add(`ez-${t}`);
          });
    }

    // Apply theme control class to host
    if (_changed.has(THEME_NAME)) {
      const old = _changed.get(THEME_NAME) as string;

      if (old) this.classList.remove(`ez-theme-${old}`);
      if (this.theme) this.classList.add(`ez-theme-${this.theme}`);
    }

    // Apply size control class to host
    if (_changed.has(SIZE_NAME)) {
      const old = _changed.get(SIZE_NAME) as string;

      if (old) this.classList.remove(`ez-${old}`);
      if (this.size) this.classList.add(`ez-${this.size}`);
    }
  ...
  #toggleIntersectState(isIntersecting: boolean) {
    if (this.intersected === isIntersecting) return;

    const _isIntersecting = (this.#intersected = isIntersecting);
  ...
  #onParentScrollUnDebounced = () => {
    ...
    // Toggle on-scroll state for M3 elevation/color changes
    this.classList.toggle('ez--scrolled', p.scrollTop > 0);
