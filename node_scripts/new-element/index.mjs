...
const __dirname = url.fileURLToPath(new URL('.', import.meta.url)),
  ...
  incomingComponentName = process.argv
  ...
  getComponentMeta = (inLocalName = '') => {
    const localName = inLocalName.startsWith('ez-')
        ? inLocalName
        : `ez-${inLocalName}`,
      classNamePrefix = localName
        .trim()
        .split('-')
        .reduce((agg, part) => agg + (!part ? '' : uCaseFirst(part)), '');

    return { localName, classNamePrefix };
  },
  doesFileExist = async filePath => fs.access(filePath, fs.constants.F_OK),
  getCustomElementContent = (localName, classNamePrefix, outputDir) => {
    const className = `${classNamePrefix}Element`,
      ...
      elementFilePath = path.join(outputDir, `${localName}.ts`),
      registerFilePath = path.join(outputDir, `register.ts`),
      indexFilePath = path.join(outputDir, `index.ts`),
...
`,
      ...
      [
        registerFilePath,
        `import {${className}} from './${localName}.ts';
import {registerCustomElement} from '../utils/index.ts';
...
import {${elementClassName}} from '@atomic/ui/${localName}/index.ts';
...
const ${className} = lazy(() => import('@atomic/ui-react/${localName}').then(mod => mod.default), {
  ssr: false
});
...
`,
  ...
  getFilesScaffolder =
    (localName, classNamePrefix) => async (filesWriter, packagePath) => {
      const outputDir = path.join(packageRoot, packagePath, localName),
        writeFiles = async () => {
          const filesAndContentsList = await filesWriter(
            localName,
            classNamePrefix,
            outputDir
          );

          if (!filesWriter.requiresAssist) return filesAndContentsList;

          // Write files
          return Promise.all(
            filesAndContentsList.map(([filePath, content]) =>
              doesFileExist(filePath).then(
                () => {
                  if (!force) {
  ...
  throwNameTypeMismatch = () => {
    throw new TypeError(
      `Element name must match pattern ${componentNameRegex}`
    );
  },
  printRelativeFilePaths = async fileNames => {
    const output = fileNames
      .reduce((agg, file) => {
        ...
        return (
          agg +
          `  - ${relativeFilePath.slice(relativeFilePath.lastIndexOf('../') + 1)}\n`
        );
      }, '')
      .trimEnd();
...
force: ${force}
  ...
  const { localName, classNamePrefix } = getComponentMeta(
  ...
  return (
    scaffoldFilesUsing(getCustomElementContent, 'packages/atomic-ui-js')
      ...
      // Print skipped files
      .then(collectedFilePaths => {
        const skippedFiles = collectedFilePaths
            .filter(fp => fp.skipped)
            .map(fp => fp.filePath),
